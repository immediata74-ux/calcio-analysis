...
export async function buildTopAnalysis({ date, league }) {
  ...
  const topDistinctFixtures = (candidates, limit = 10) => {
  const seen = new Set();
  const result = [];

  for (const candidate of [...candidates].sort((a, b) => b.percent - a.percent)) {
    const key = candidate.fixtureId || candidate.fixture;
...
const cards = topDistinctFixtures(cardCandidates, 10).map(publicSignal);
const scorers = topDistinctFixtures(scorerCandidates, 10).map(publicSignal);
