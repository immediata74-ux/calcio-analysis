...
const MAX_COVERAGE_SCAN_LEAGUES = 24;
const MAX_ALL_LEAGUES = 8;
const MAX_CORNER_LEAGUES_ALL = 4;
const RECENT_CORNER_MATCHES = 7;
const CORNER_LOOKBACK_DAYS = 180;
const MAX_DEEP_TEAMS_ALL = 12;
const MAX_DEEP_TEAMS_SINGLE = 20;
...
function supportScore(group) {
  ...
  return (coverage.top_cards === true ? 3 : 0)
    + (coverage.top_scorers === true ? 3 : 0)
    + (coverage.fixtures?.statistics_fixtures === true ? 2 : 0)
...
export async function buildTopAnalysis({ date, league }) {
  ...
  // Prima controlliamo una rosa più ampia di competizioni, poi scegliamo quelle
  // con copertura utile. In v0.4 sceglievamo le prime 8 per numero di partite:
  // questo favoriva leghe minori senza statistiche e poteva produrre Top vuoti.
  const coverageCandidates = leagueFilter ? groups.slice(0, 1) : groups.slice(0, MAX_COVERAGE_SCAN_LEAGUES);
  const coverageScanned = await mapLimit(coverageCandidates, 5, async group => {
    try {
  ...
  const withCoverage = leagueFilter
    ? coverageScanned
    : coverageScanned.sort((a, b) => supportScore(b) - supportScore(a)).slice(0, MAX_ALL_LEAGUES);
  ...
  // I Top API-Football contengono solo 20 giocatori per lega. Se le squadre
  // del palinsesto non hanno un giocatore in quei 20, v0.4 restava vuota.
  // v0.5 fa quindi un fallback prudente sulle statistiche stagionali delle
  // squadre effettivamente in campo, con paginazione limitata per proteggere quota e latenza.
  const deepTargets = deepTeamTargets(withCoverage, leagueFilter);
  const deepFetched = await mapLimit(deepTargets, 4, async target => {
    try {
      ...
      return { group: target.group, cards: [], scorers: [] };
    }
  });
  ...
  const cornerCovered = coverageScanned
    .filter(g => g.coverage?.fixtures?.statistics_fixtures === true)
  ...
  return {
    ...
    meta: {
      ...
      note: 'v0.5: fallback sulle statistiche dei giocatori delle squadre in palinsesto; percentuali solo su dati API-Football disponibili. I null restano null e i campioni insufficienti sono esclusi.',
    },
  };
}
