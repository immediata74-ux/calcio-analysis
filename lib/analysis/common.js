...
export function expectedMinutes(stat) {
  ...
  if (!Number.isFinite(minutes) || !Number.isFinite(appearances) || appearances <= 0) {
    return null;
  }

  const average = clamp(minutes / appearances, 15, 90);

  let reliability = 1;

  if (appearances <= 3 && minutes < 450) {
    reliability = 0.65;
  } else if (appearances <= 5 && minutes < 600) {
    reliability = 0.80;
  } else if (appearances <= 7 && minutes < 720) {
    reliability = 0.90;
  }

  return clamp(average * reliability, 15, 90);
}
